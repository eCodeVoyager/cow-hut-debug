...
const getAllSeller = async (paginationOptions: IpaginationOptions, filters: ISellerFilters): Promise<IGenericResponse<ISeller[]>> => {
    ...
    const whereCondition = SellerSearchableFileds.length > 0 ? { $and: searchAndFilterCondition } : {};

    const result = Seller.find(whereCondition).sort(sortCondition).skip(skip).limit(limit);
